...
CRON_LOGS_PATH="$INSTAGRAM_PLAYWRIGHT_PATH/cron/cron-triggers.log"
...
# Trigger -> Run check
LAST_SUCCESSFUL_TIMESTAMP=$(grep "$LOG_CRON_SUCCESS_TIME_PREFIX" "$CRON_LOGS_PATH" | tail -n 1 | awk -F"$LOG_CRON_SUCCESS_TIME_PREFIX" '{print $2}')
...
if [[ "$DIFF_HOURS" -lt "$MIN_HOURS_BETWEEN_TRIGGERS" ]]; then
    echo "[$(date +"$LOG_DATE_FORMAT")] Skipping notifications as not enough time has elapsed before next post." >>"$CRON_LOGS_PATH"
    exit 0
else
    echo "Running cron at: $(date +"$LOG_DATE_FORMAT")" >>"$CRON_LOGS_PATH"
fi
